...
class Calculadora(tk.Tk):
    def __init__(self):
        super().__init__()
        self.geometry('400x450')  # Esto sirve para implementar la resolucion de la venta
        self.resizable(0, 0)  # Esto sirve para que no se pueda cambiar el ancho y alto de la ventana
        self.title('Calculadora')
        self.iconbitmap('calculadora.ico')
        # Atributos de clase
        self.expresion = ''
        # Caja de texxto(input)
        self.entrada = None
    ...
    def _creacion_componentes(self):
        ...
        boton_multiplicar = tk.Button(botones_frame, text='*', width=10, height=3,
                                      bd=0, bg='#eee', cursor='hand2',
                                      command=lambda: self._evento_click('*'))
        ...
        boton_sumar = tk.Button(botones_frame, text='+', width=10, height=3,
                                bd=0, bg='#eee', cursor='hand2',
                                command=lambda: self._evento_click('+')).grid(
        ...
        boton_restar = tk.Button(botones_frame, text='-', width=10, height=3,
                                 bd=0, bg='#eee', cursor='hand2',
                                 command=lambda: self._evento_click('-'))
        boton_restar.grid(row=3, column=3, padx=1, pady=1)

        # Quinto y ultimo renglon
    ...
    def _evento_evaluar(self):
        # eval evalua la expresion de tipo string como una expresion aritmetica
        try:
            if self.expresion:
